...
//busca nome do cliente antes de excluir
	...
	$linhaEst = str_pad($busca['id_est_mtp'], 9, "0", STR_PAD_LEFT);
...
//fim da busca de nome
	...
	if (!mysqli_query($conn, "DELETE FROM estoque_mtp WHERE id_est_mtp='$codEx'")) {
		...
		$textA = 'text-white';
		$iconA = 'far fa-thumbs-down';
		$descA = 'Erro ao excluir material do estoque!';

		include '../includes/alerta.php';

		$_SESSION['msg'] = $alertaSess;

		header("location: ../listagem_total_estMtp&pagina=" . $paginaR);

	} else {

		$conn->close();

		$acao = 'Deletou a refêrencia ' . $linhaEst . ' do estoque de materiais';
		...
		$descA = 'Material excluído do estoque com sucesso!';
