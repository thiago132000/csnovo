...
//fim da busca de nome
	...
	$buscarDep = "SELECT * FROM departamento WHERE id=?";

	if (!$stmt = $conn->prepare($buscarDep)) {
		die("Erro: " . $conn->erro);
	}
	...
	if (!$result->num_rows == 0) {
		...
		mysqli_query($conn, "DELETE FROM departamento WHERE id='$codEx'");

		$conn->close();

		$acao = 'Deletou o departamento ' . $nomeDep;
		...
		$descA = 'Departamento excluído com sucesso!';

		include '../includes/alerta.php';

		$_SESSION['msg'] = $alertaSess;

		header("location: ../listagem_total_dep&pagina=" . $paginaR);
		...
		$textA = 'text-white';
		$iconA = 'far fa-thumbs-down';
		$descA = 'Erro ao excluir o departamento!';
